...
if __name__ == '__main__':
    ...
    image = imagepreprocessing.preprocess_image(image, False)
    ...
    '''
    corners = imagepreprocessing.find_corners_of_sudoku(image)

    '''
        Step 3: Crop Image -> using wrap
    '''
    cropped_image= imagepreprocessing.crop_image(image,corners)
    cropped_img = cv2.resize(cropped_image, (500, 500))              # Resize image
    #show_image(cropped_img,"Cropped")

    '''
        Step 4: Get Squares from the cropped image of sudoku
    '''
    number_of_squares = 9
    sudoku_squares = imagepreprocessing.get_squares_coordinates(cropped_image,number_of_squares)

    '''
        Step 5: Get digits from each square of sudoku
    '''
    size_of_digit_image = 28 # as per MNIST dataset
    sudoku_digits = imagepreprocessing.get_digits(cropped_image, sudoku_squares, size_of_digit_image)   
    #print(sudoku_digits)

    '''
        Step 6: Show Digits
    '''
    final_image = imagepreprocessing.show_digits(sudoku_digits)
    ...
    grid = imagepreprocessing.extract_number(final_image, loaded_model)
    print(grid)